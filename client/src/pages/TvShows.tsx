...
const TvShows = () => {
  ...
  const { data: heroContent, isLoading: isLoadingHero } = useContentByFeature('Type Hero');
  const { data: newReleaseContent, isLoading: isLoadingNewRelease } = useContentByFeature('Type New Release');
  const { data: popularContent, isLoading: isLoadingPopular } = useContentByFeature('Type Popular');
  const { data: entertainmentContent, isLoading: isLoadingEntertainment } = useContentByFeature('Entertainment');
  const { data: fictionalContent, isLoading: isLoadingFictional } = useContentByFeature('Fictional');

  // Fetch content by genre
  const { data: dramaContent, isLoading: isLoadingDrama } = useContentByGenre('Drama');

  const isLoading = isLoadingAll || isLoadingHero || isLoadingNewRelease || 
                   isLoadingPopular || isLoadingEntertainment || isLoadingFictional || isLoadingDrama;
  ...
  const getShowsFromContentArray = (contentArray: any[] | undefined) => {
  ...
  const sections = [
    { title: 'New Release', contents: getShowsByFeature('Type New Release') },
    { title: 'Popular', contents: getShowsByFeature('Type Popular') },
    { title: 'Entertainment', contents: getShowsByFeature('Entertainment') },
    { title: 'Drama', contents: getShowsFromContentArray(dramaContent) },
    { title: 'Fictional', contents: getShowsByFeature('Fictional') },
  ];
        ...
        {sections.map((section, index) => (
          section.contents.length > 0 && (
            <HorizontalSection
              ...
              onSeeMore={() => {
                console.log(`See more ${section.title} shows`);
              }}
            />
          )
        ))}
